...
const SignUp = () => {
    ...
    const handleSubmit = (event) => {
        ...
        http.post("user/create", payload)
            .then(response => {
                if (response.ok) alert("You have successfully Signed Up, please use your credentials to Sign In!")
                throw response
            })
    ...
    return (
        <React.Fragment>
            {openSignUp ? <CustomModal modalOpen={openSignUp}
                onModalClose={() => { setOpenSignUp(false) }}
                children={getSignUpForm()}
            /> : null}
            <div style={{ display: "flex", alignItems: "center", padding: "10px" }}>
                <Button style={{ color: "white" }} onClick={() => { setOpenSignUp(true) }}>
                    Sign Up<PersonAddAltIcon style={{ color: "white" }} />
                </Button>
            </div>
        </React.Fragment>
    );
