...
const SignIn = () => {
    ...
    const handleSubmit = (event) => {
        ...
        http.postForm("login", formData)
            ...
            .then(data => {
                appContext.setAuthToken(data.access_token);
                window.sessionStorage.setItem("authToken", data.access_token);

                appContext.setAuthTokenType(data.token_type);
                window.sessionStorage.setItem("authTokenType", data.token_type);

                appContext.setUsername(data.username);
                window.sessionStorage.setItem("username", data.username);

                appContext.setUserId(data.user_id);
    ...
    return (
        <React.Fragment>
            {openSignIn ? <CustomModal modalOpen={openSignIn}
                onModalClose={() => { setOpenSignIn(false) }}
                children={getSignInForm()}
            /> : null}
            <div style={{ display: "flex", alignItems: "center", padding: "10px" }}>
                <Button style={{ color: "white" }} onClick={() => { setOpenSignIn(true) }}>
                    Sign In<LoginIcon style={{ color: "white" }} />
                </Button>
            </div>
        </React.Fragment>
    )
